...
def filter_lock_to_hosted(lock_content):
    """Reduce a pub lockfile to a standalone lockfile of its hosted entries.

    Flutter >= 3.35 resolves its SDK packages as a pub *workspace*: every
    package declares `any` constraints and one lockfile at the SDK root
    (`flutter/pubspec.lock`, checked into flutter/flutter) pins the whole
    monorepo. Because rules_flutter resolves each SDK package standalone (see
    _sanitize_published_pubspec, which drops the `resolution: workspace`
    marker), those `any` constraints would otherwise resolve to whatever
    pub.dev serves as latest at fetch time — so the vendored cache silently
    changed version whenever a transitive dependency published, and no longer
    matched the versions consumers pinned in their own pub_deps.json.

    Seeding each package with these pins restores Flutter's resolution.
    Non-hosted entries are dropped: the workspace lock also carries git and
    `sdk:` sources for monorepo-internal packages (dev/integration_tests/...)
    that a standalone package never resolves. Extra hosted entries are
    harmless — pub keeps the ones a package needs and prunes the rest.

    Args:
        lock_content: Contents of the SDK's workspace `pubspec.lock`, or "".

    Returns:
        A lockfile holding only the hosted entries, or "" when there is
        nothing to seed.
    """
    if not lock_content:
        return ""

    # (header line, [body lines]) per entry, in file order.
    entries = []
    current = None
    in_packages = False
    for line in lock_content.split("\n"):
        ...
        if indent == 0:
            # A new top-level key (`packages:`, `sdks:`) ends any entry.
    ...
    for entry in entries:
        for body_line in entry[1]:
            if body_line.strip() == "source: hosted":
                kept.append(entry[0])
                kept.extend(entry[1])
                break
    ...
    # The `sdks:` block is deliberately not carried over: it pins the SDK
    # range the *workspace* resolved under, and a package whose own
    # `environment:` is narrower would make pub discard the whole lockfile.
    return "\n".join([
        "# Generated by rules_flutter from the Flutter SDK workspace lock.",
        "packages:",
    ] + kept) + "\n"

def _ensure_pub_deps(repository_ctx, package_name, package_dir, allow_fallback_on_failure = False, resolve_deps = True, lock_seed = ""):
    ...
    # Pin the solve to the versions the SDK itself resolved (see
    # filter_lock_to_hosted). Only when the package ships no lock of its own:
    # pre-workspace Flutter releases pin exact versions in every package
    # pubspec and need no seed at all.
    if lock_seed and not repository_ctx.path(lock_rel).exists:
        repository_ctx.file(lock_rel, lock_seed)
...
def generate_package_build(repository_ctx, package_name, package_dir = ".", sdk_repo = "@flutter_sdk", include_hosted_deps = True, include_pub_cache_data = False, hosted_deps = None, resolve_deps = True, lock_seed = ""):
    ...
    Args:
        ...
        lock_seed: Optional lockfile (from filter_lock_to_hosted) written
            before the solve so it resolves to already-pinned versions
            instead of whatever pub.dev serves as latest at fetch time.
    """
