...
def _toolchains_repo_impl(repository_ctx):
    if "+" not in repository_ctx.name:
        fail("Unexpected canonical repo name '{}' for Flutter toolchains".format(repository_ctx.name))
    canonical_prefix, _ = repository_ctx.name.rsplit("+", 1)
...
"""
    ...
    for [platform, meta] in PLATFORMS.items():
        canonical_target_repo = "{}+{}_{}".format(
            canonical_prefix,
            repository_ctx.attr.user_repository_name,
            platform,
        )
        build_content += """
