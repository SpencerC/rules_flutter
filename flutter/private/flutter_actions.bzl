...
def flutter_pub_get_action(ctx, flutter_toolchain, working_dir, pubspec_file):
    ...
    Returns:
        Tuple of (pub_get_output, pub_cache_file, pubspec_lock, dart_tool_file)
    """
    ...
    pub_cache_file = ctx.actions.declare_file(ctx.label.name + "_pub_cache/cache.json")
    pubspec_lock = ctx.actions.declare_file(ctx.label.name + "_pubspec.lock")
    dart_tool_file = ctx.actions.declare_file(ctx.label.name + "_dart_tool/package_config.json")
...
PUB_CACHE_FILE="{pub_cache_file}"
FLUTTER_BIN="{flutter_bin}"
ORIGINAL_PWD="$PWD"

# Set up pub cache directory from file path
PUB_CACHE_DIR="$(dirname "$PUB_CACHE_FILE")"
...
if "$FLUTTER_BIN_ABS" pub get --suppress-analytics; then
    ...
    if [ -d ".dart_tool" ]; then
        ...
        if [ -f ".dart_tool/package_config.json" ]; then
            ...
            echo "Packages configured: $(jq '.packages | length' .dart_tool/package_config.json 2>/dev/null || echo 'unknown')"
        else
    ...
    # Check pub cache
    if [ -n "$(ls -A "$PUB_CACHE" 2>/dev/null)" ]; then
...
""".format(
        ...
        pub_cache_file = pub_cache_file.path,
        flutter_bin = flutter_bin,
    ...
    ctx.actions.run_shell(
        ...
        outputs = [pub_get_output, pubspec_lock, pub_cache_file, dart_tool_file],
...
mkdir -p "$(dirname "{pub_cache_file}")"
mkdir -p "$(dirname "{dart_tool_file}")"
...
# Copy package_config.json from .dart_tool (should always exist after successful pub get)
if [ -f "$WORKSPACE_DIR/.dart_tool/package_config.json" ]; then
    echo "✓ Copying .dart_tool/package_config.json from Flutter pub get" >> "{pub_get_output}"
    cp "$WORKSPACE_DIR/.dart_tool/package_config.json" "{dart_tool_file}"
else
    echo "✗ FATAL ERROR: .dart_tool/package_config.json not found after successful pub get" >> "{pub_get_output}"
    echo "This should never happen - Flutter pub get validation failed"
    exit 1
fi

# Create pub cache metadata
if [ -d "$PUB_CACHE" ] && [ -n "$(ls -A "$PUB_CACHE" 2>/dev/null)" ]; then
    echo "✓ Pub cache populated with dependencies" >> "{pub_get_output}"
    echo '{{"cache_populated": true, "timestamp": "'$(date -Iseconds)'"}}' > "{pub_cache_file}"
else
    echo "⚠ Pub cache directory empty or not found" >> "{pub_get_output}"
    echo '{{"cache_populated": false, "timestamp": "'$(date -Iseconds)'"}}' > "{pub_cache_file}"
fi
...
""".format(
            ...
            pub_cache_file = pub_cache_file.path,
            dart_tool_file = dart_tool_file.path,
            flutter_bin = flutter_bin,
        ),
    ...
    return pub_get_output, pub_cache_file, pubspec_lock, dart_tool_file

def flutter_build_action(ctx, flutter_toolchain, working_dir, target, pub_cache_file, dart_tool_file):
    ...
    Args:
        ...
        pub_cache_file: Pub cache file from pub get
        dart_tool_file: Dart tool file from pub get
...
# Copy dart_tool file to workspace directory structure
if [ -f "$DART_TOOL_FILE" ]; then
    mkdir -p .dart_tool
    cp "$DART_TOOL_FILE" .dart_tool/package_config.json
fi
...
""".format(
    ...
    # Execute build
    ctx.actions.run_shell(
        inputs = [working_dir, pub_cache_file, dart_tool_file] + flutter_toolchain.flutterinfo.tool_files + flutter_toolchain.flutterinfo.sdk_files,
...
def flutter_test_action(ctx, flutter_toolchain, working_dir, test_files, pub_cache_file, dart_tool_file):
