...
DartLibraryInfo = provider(
    ...
    fields = {
        ...
        "pub_cache": "The pub cache directory from pub get (optional)",
        "transitive_pub_caches": "Depset of pub cache directories from all transitive dependencies",
    },
)
...
if [ -f "$DART_BIN" ]; then
    "$DART_BIN" pub get 2>&1 > /dev/null || echo "Warning: dart pub get failed, continuing anyway"
else
    "$FLUTTER_BIN_ABS" pub get --suppress-analytics 2>&1 > /dev/null || echo "Warning: flutter pub get failed, continuing anyway"
fi
popd >/dev/null
...
def _dart_library_impl(ctx):
    ...
    # If pubspec is provided, run dart pub get to generate pub_deps.json
    pubspec_file = ctx.file.pubspec
    pub_deps = None
    pub_get_output = None
    pub_cache_dir = None
    ...
    if pubspec_file:
        ...
        # Run pub get to generate pub_deps.json
...
_dart_library_rule = rule(
    ...
    attrs = {
        ...
        "deps": attr.label_list(
            ...
            providers = [[DartLibraryInfo], [FlutterLibraryInfo]],
        ),
        "pubspec": attr.label(
            allow_single_file = True,
