...
- AGENTS: Do not treat any task as complete until `bazel test //flutter/tests:all_tests //docs:update_tests` passes from the repo root and every test in the e2e smoke workspace passes (`cd e2e/smoke && bazel test //:integration_tests`). The docs gate must be named explicitly — it is manual-tagged (its goldens match only the pinned Bazel's stardoc rendering).
